(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('bloggingApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-bloggingApp-error');
                var entityKey = httpResponse.headers('X-bloggingApp-params');
                if (errorHeader) {
